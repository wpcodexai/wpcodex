<?php
/**
 * Ability: wpworker/gutenberg-enable-batch-finalization
 *
 * @package WPWorker
 * @since   1.0.0
 */
...
class EnableFinalization extends AbstractAbility {
	...
	public function get_name(): string {
		return 'wpworker/gutenberg-enable-batch-finalization';
	}

	/** {@inheritDoc} */
	public function get_label(): string {
		return __( 'Enable Gutenberg Batch Finalization', 'worker-ai' );
	}

	/** {@inheritDoc} */
	public function get_description(): string {
		return __( 'Transitions a draft Gutenberg batch to ready so the Block Editor Queue can begin finalization. The browser-side JS runtime picks it up automatically when the queue page is open.', 'worker-ai' );
	}
	...
	public function get_instructions(): string {
		return implode( "\n", [
			'Call after wpworker/gutenberg-add-pending-change to transition the batch from draft to ready.',
	...
	public function execute( array $input ): array|\WP_Error {
		...
		if ( $items === [] ) {
			return new \WP_Error(
				...
				sprintf( 'Gutenberg batch %d has no items. Add at least one change with wpworker/gutenberg-add-pending-change.', $batch->ID ),
				[ 'status' => 409 ]
			);
		}
