...
document.addEventListener( 'DOMContentLoaded', () => {
	...
	page.querySelectorAll( '#wpworker-manual-tabs .wpworker-client-tab' ).forEach(
		( tab ) => {
			tab.addEventListener( 'click', () => {
				manualClient = tab.dataset.client;
				page.querySelectorAll(
					'#wpworker-manual-tabs .wpworker-client-tab'
				).forEach( ( t ) =>
	...
	page.querySelectorAll( '.wpworker-npxless-tab' ).forEach( ( tab ) => {
		tab.addEventListener( 'click', () => {
			...
			tab.closest( '.wpworker-client-tabs' )
				?.querySelectorAll( '.wpworker-client-tab' )
				.forEach( ( t ) =>
					t.classList.toggle( 'is-active', t === tab )
				);
			renderNpxless();
	...
	window.wpworkerUpdateName = ( value ) => {
		...
		if ( suggestion )
			suggestion.style.display =
				value.trim().length > 0 &&
				! value.toLowerCase().includes( 'wpworker' )
					? ''
					: 'none';
		renderAll();
	};
	...
	window.wpworkerToggleServerName = ( btn ) => {
		...
		if ( ! expanded )
			document.getElementById( 'wpworker-mcp-name' )?.focus();
	};
	...
	window.wpworkerCopyPaste = ( btn ) => {
		const text =
			document.getElementById( 'wpworker-paste-text' )?.textContent || '';
	 ...
	 * @param  {HTMLElement} btn The copy button element.
	 */
	window.wpworkerCopyNpxless = ( btn ) =>
		copyEl( 'wpworker-npxless-code', btn );
		...
		fetch( AJAX_URL, {
			...
			.then( ( data ) => {
				...
				if ( tbody ) {
					...
					tr.innerHTML =
						`<td class="wpworker-pw-table__name">${ displayName }</td>` +
						`<td class="wpworker-pw-table__meta">${ data.data.created }</td>` +
						`<td class="wpworker-pw-table__meta">${
							L10N.never || 'Never'
						}</td>` +
						`<td class="wpworker-pw-table__actions"><button type="button" class="button button-small wpworker-pw-revoke-btn" onclick="wpworkerRevokePassword('${
							data.data.uuid
				...
				if ( existing ) {
					...
					const countEl =
						document.getElementById( 'wpworker-pw-count' );
					if ( countEl ) {
						const tbody2 =
							document.getElementById( 'wpworker-pw-tbody' );
						countEl.textContent = `(${
