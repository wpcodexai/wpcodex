=== AllyWorker — The AI operating system for WordPress developers ===
Contributors: allyworker
...
= What AI agents can do with AllyWorker (free) =
...
* **Manage sandbox files** — enable or disable PHP snippets in `wp-content/allyworker-sandbox/` without touching theme files
...
* Created and edited from **AllyWorker → Skills** in the admin
...
Writing Gutenberg blocks requires the JavaScript block registry running in a real browser. AllyWorker solves this with a queue-and-finalize flow:

1. The agent queues block changes via the `allyworker/gutenberg-write-content` ability
...
Every ability requires the `manage_options` capability (or super-admin on Multisite). Authentication uses WordPress Application Passwords — native WordPress, revocable per client, no separate secret key. You can individually disable any ability from **AllyWorker → Ability Settings** without disabling the whole plugin.

The sandbox directory (`wp-content/allyworker-sandbox/`) isolates PHP snippets from the rest of the codebase. Files that throw a fatal error are auto-disabled so they do not break the site.
...
After activating the plugin, go to **AllyWorker → Configuration** and follow the three steps:
...
See the full [Get Started guide](https://allyworker.com/docs/get-started/) for detailed instructions per client.

== Frequently Asked Questions ==

= What is the difference between AllyWorker free and AllyWorker Pro? =

The free plugin gives agents read-only and structured-write access: reading files and site state, querying posts, managing options, uploading files, writing Gutenberg blocks, and managing Skills. [AllyWorker Pro](https://allyworker.com/pro/) adds the most powerful development abilities — arbitrary PHP execution, WP-CLI, direct SQL, and full filesystem write/edit/delete access. Pro is intended for development and staging environments where those capabilities are needed.

= Is AllyWorker safe to use on a production site? =

The free plugin does not provide arbitrary code execution or unrestricted filesystem write access, so the risk profile is significantly lower than Pro. That said, any plugin that allows remote agents to set WordPress options or upload files should be used with awareness. Revoke Application Passwords for any agent you no longer use, and review **AllyWorker → Ability Settings** to disable abilities you do not need.
...
Go to **AllyWorker → Configuration**, scroll to the password table in Step 2, and click **Revoke** next to the password you want to remove. You can also revoke from **Users → Profile → Application Passwords**.

= Can I restrict which tools the agent can use? =

Yes. Go to **AllyWorker → Ability Settings** to enable or disable individual abilities. For example, you can allow file reads but block option writes, or disable the upload link ability entirely.
...
= Does AllyWorker send data to any external service? =

No. AllyWorker only communicates between your server and the AI client connecting to it. There is no telemetry, no external API calls, and no hosted service.

= Where is the sandbox directory? =

The sandbox directory is `wp-content/allyworker-sandbox/`. PHP files placed here are loaded automatically on every WordPress request. Files that throw a fatal error are auto-disabled so they do not break the site. PHP file uploads via `create-upload-link` are restricted to this directory.
...
Yes. On multisite, only super admins can access AllyWorker settings and call abilities. Individual site administrators do not have access.
...
3. Ability Settings — enable or disable individual abilities, including Pro abilities when AllyWorker Pro is active
4. Block Editor Queue — review and finalize Gutenberg content changes queued by the agent
5. The admin bar "AllyWorker ON" indicator — a persistent reminder that AI abilities are active

== Changelog ==

= 0.6.0 =
* Moved `php-execute`, `wpcli-run`, `db-query`, `file-write`, `file-edit`, and `file-delete` to [AllyWorker Pro](https://allyworker.com/pro/)
* Free plugin now focuses on read, inspect, structured-write, Skills, Gutenberg, and sandbox management abilities
* Added Pro ability section to Ability Settings page (shows Pro badge when AllyWorker Pro is not active)
...
= 0.6.0 =
The `php-execute`, `wpcli-run`, `db-query`, `file-write`, `file-edit`, and `file-delete` abilities have moved to AllyWorker Pro. If you rely on these, install AllyWorker Pro before upgrading.
