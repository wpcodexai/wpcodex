 ...
 * response includes WPWorker environment instructions and the skill catalog.
...
namespace WPWorker\Abilities\Core;
...
class DiscoverAbilities extends AbstractAbility {
	...
	public function get_label(): string {
		return __( 'Discover Abilities', 'worker-ai' );
	}

	/** {@inheritDoc} */
	public function get_description(): string {
		return __(
			'Discover all available WordPress abilities. Returns the full ability list plus WPWorker environment instructions and skill catalog. Call this at the start of every session.',
			'wpworker'
		);
	}
	...
	public function get_output_schema(): array {
		return [
			...
			'properties' => [
				'instructions' => [
					'type'        => 'string',
					'description' => 'WPWorker environment instructions and skill catalog for this session.',
				],
				'abilities'    => [
					'type'  => 'array',
					'items' => [
	...
	public function check_permission(): bool|\WP_Error {
		if ( ! is_user_logged_in() ) {
			return new \WP_Error( 'wpworker_not_authenticated', 'Authentication required.', [ 'status' => 401 ] );
		}
		/** @var string $cap */
		$cap = apply_filters( 'wpworker_discover_abilities_capability', 'read' );
		if ( ! current_user_can( $cap ) ) {
			return new \WP_Error( 'wpworker_insufficient_capability', 'Insufficient capability.', [ 'status' => 403 ] );
		}
		return true;
	}
	...
	private static function build_instructions(): string {
		...
		$lines = [
			'## WPWorker — AI Operating System for WordPress',
			'',
			'You are connected to a WordPress site via the WPWorker MCP server.',
			'',
			'**Site:** ' . $site_url,
		...
		$lines = array_merge( $lines, [
			'### Abilities',
			'',
			'- Execute arbitrary PHP with `wpworker/php-execute` — full WordPress environment available (`$wpdb`, all functions, all plugins).',
			'- Run WP-CLI commands with `wpworker/wpcli-run`.',
			'- Read, write, edit, delete, and list files with the `wpworker/file-*` abilities.',
			'- Persist PHP code across requests by writing files to the sandbox directory and using `wpworker/file-disable` / `wpworker/file-enable` to control loading.',
			'- Manage skill playbooks with `wpworker/skill-*` abilities.',
			'',
		] );
		...
		$lines = array_merge( $lines, [
			'## Rules',
			'',
			'- Always inspect before modifying. Use `wpworker/file-read` or `wpworker/php-execute` to understand the current state.',
	 ...
	 * (user DB + external plugin sources via the wpworker_skill_sources filter).
	 */
	private static function build_skill_catalog(): string {
		...
		$lines = [
			'## Available Skills',
			'',
			'Call `wpworker/skill-read` with the skill name to load its full instructions.',
			'',
		];
