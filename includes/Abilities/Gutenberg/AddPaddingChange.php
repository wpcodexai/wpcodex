<?php
/**
 * Ability: wpworker/gutenberg-add-pending-change
 *
 * @package WPWorker
 * @since   1.0.0
 */
...
class AddPaddingChange extends AbstractAbility {
	...
	public function get_name(): string {
		return 'wpworker/gutenberg-add-pending-change';
	}

	/** {@inheritDoc} */
	public function get_label(): string {
		return __( 'Add Gutenberg Pending Change', 'worker-ai' );
	}

	/** {@inheritDoc} */
	public function get_description(): string {
		return __( 'Adds one Gutenberg block change to a pending batch. If batch_id is omitted a new draft batch is created automatically. The operation is queued — changes are not live until the batch is finalized through the Block Editor Queue.', 'worker-ai' );
	}
	...
	public function get_instructions(): string {
		return implode( "\n", [
			...
			'After adding all changes, call wpworker/gutenberg-enable-batch-finalization to transition the batch from draft to ready and unlock the Block Editor Queue.',
	...
	public function execute( array $input ): array|\WP_Error {
		...
		if ( $target_id <= 0 ) {
			return new \WP_Error( 'wpworker_invalid_input', __( 'target_id (or post_id) must be a positive integer.', 'worker-ai' ) );
		}
		...
		if ( ! $target instanceof \WP_Post ) {
			return new \WP_Error(
				...
				sprintf( __( 'Target post %d was not found.', 'worker-ai' ), $target_id )
			);
		}
		...
		if ( $batch_id_input > 0 ) {
			...
			if ( ! $batch instanceof \WP_Post ) {
				return new \WP_Error(
					...
					sprintf( __( 'Gutenberg batch %d was not found.', 'worker-ai' ), $batch_id_input )
				);
			}
