<?php
/**
 * Ability: allyworker/gutenberg-list-pending-batches
 *
 * @package AllyWorker
 * @since   1.0.0
 */
...
class ListPadding extends AbstractAbility {
	...
	public function get_name(): string {
		return 'allyworker/gutenberg-list-pending-batches';
	}

	/** {@inheritDoc} */
	public function get_label(): string {
		return __( 'List Gutenberg Pending Batches', 'allyworker' );
	}

	/** {@inheritDoc} */
	public function get_description(): string {
		return __( 'Lists compact queue state grouped by Gutenberg batch for agent recovery, plus the current Block Editor Queue runtime status and curl SSE/poll URLs. Full block specs are not returned.', 'allyworker' );
	}
	...
	public function get_instructions(): string {
		return implode( "\n", [
			...
			'For one batch, call allyworker/gutenberg-get-pending-batch, then watch the returned sse_url with curl -N or poll_url with curl.',
		] );
	}
