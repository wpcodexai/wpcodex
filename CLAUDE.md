...
1. Call `wpcodex/site-info` — get the live install snapshot (WP version, PHP version, active plugins, active theme, site URLs).
...
| `wpcodex/skill-create` | Creates a new skill. Pass `name`, `description`, `body`, optional `enable_agentic` (bool), `enable_prompt` (bool). |
| `wpcodex/skill-update` | Updates an existing skill. Pass `name` and any fields to change. |
| `wpcodex/skill-delete` | Deletes a skill. Pass `name`. |
...
- `wpcodex/skill-list` returns names and descriptions — read these to understand what standing instructions exist
- Load a skill body with `wpcodex/skill-read` before performing the relevant task
