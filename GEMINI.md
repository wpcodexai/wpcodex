# WPCodex — Agent Context (Gemini CLI)

## What This Is

WPCodex gives AI agents **unrestricted control over a WordPress installation** via an MCP server plugin. With arbitrary PHP execution, WP-CLI access, full filesystem access, and database query support, an agent can do *anything* WordPress can do — install plugins, modify themes, query the database, call external APIs, and build custom functionality on the fly.
...
WPCodex uses the official `wordpress/mcp-adapter` for MCP transport. Authentication is via **WordPress Application Passwords** — no separate secret key.

```
Endpoint : POST /wp-json/mcp/wpcodex
Protocol : MCP 2025-06-18 over HTTP (wordpress/mcp-adapter)
Auth     : Authorization: Basic base64(username:app-password)
```

Create an Application Password at **WordPress Admin → Users → Your Profile → Application Passwords**. The full connection config for your client is shown on **WPCodex → Connect**.
...
1. Call `wpcodex/site-info` — get the live install snapshot (WP version, PHP version, active plugins, theme, site URLs).
2. Call `wpcodex/skill-list` — retrieve all skill names and descriptions.
3. For every skill where `enable_agentic` is `true`, call `wpcodex/skill-read` to load the full body. These are your standing instructions for this site.
...
|---|---|
| `wpcodex/php-execute` | Run PHP inside the WordPress process. Pass `code` (string, no opening tag). |
| `wpcodex/wpcli-run` | Execute WP-CLI. Pass `command` without the leading `wp`. Optional `timeout` (int, seconds). |
| `wpcodex/db-query` | Run SQL via `$wpdb`. Pass `sql` and optional `args` array. SELECT returns rows as JSON; mutations return affected row count. |
...
| Ability | What it does |
|---|---|
| `wpcodex/skill-list` | Returns all skills with `name`, `description`, `enable_agentic`, `enable_prompt`. |
| `wpcodex/skill-read` | Returns the full body of a skill. Pass `name`. |
| `wpcodex/skill-create` | Creates a new skill. Pass `name`, `description`, `body`, optional `enable_agentic` and `enable_prompt` (bool). |
| `wpcodex/skill-update` | Updates an existing skill. Pass `name` and any fields to change. |
| `wpcodex/skill-delete` | Deletes a skill by name. Pass `name`. |

---

## Skills System

Skills are Markdown playbooks stored in the **WordPress database**. They are **not** files on disk — do not try to read them with `wpcodex/file-read`.

The agent reads only skill **descriptions** at session start (cheap). When your prompt matches a description, load the full body with `wpcodex/skill-read`. Skills with `enable_agentic: true` should always be loaded.

After completing a complex task, document what you learned by calling `wpcodex/skill-create`:
...
├── includes/            # PSR-4 root (namespace: WPCodex\)
│   ├── Plugin.php
...
- **Namespace root:** `WPCodex\` → `includes/`
...
- **Hook names:** prefixed `wpcodex_` — e.g. `add_action('wpcodex_after_activate', ...)`
- **Option names:** prefixed `wpcodex_` — e.g. `get_option('wpcodex_setting_name')`
