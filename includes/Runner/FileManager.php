 ...
 * Class FileManager
 *
 * Provides structured filesystem operations for the WPWorker file abilities.
...
class FileManager {
	...
	private const BACKUP_TRANSIENT_PREFIX = 'wpworker_bak_';

	/** Extensions / basenames that must be written inside WPWORKER_SANDBOX_DIR. */
	...
	public function read_file( string $path, int $offset = 0, int $limit = 1_048_576 ): array {
		...
		if ( ! is_readable( $path ) ) {
			throw new \RuntimeException(
				sprintf(
					/* translators: %s: File path. */
					__( 'File not readable: %s', 'worker-ai' ),
					esc_html( $path )
				)
			);
		}
		...
		if ( false === $fh ) {
			throw new \RuntimeException(
				/* translators: %s: file path */
				sprintf( __( 'Failed to open: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
		...
		if ( false === $raw ) {
			throw new \RuntimeException(
				/* translators: %s: file path */
				sprintf( __( 'Failed to read: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
	...
	public function write_file(
		...
		if ( is_link( $path ) ) {
			throw new \InvalidArgumentException(
				/* translators: %s: file path */
				sprintf( __( 'Refusing to write through symlink: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
		...
		if ( 'base64' === $encoding ) {
			...
			if ( false === $decoded ) {
				throw new \RuntimeException( __( 'Content is not valid base64.', 'worker-ai' ) );
			}
			$content = $decoded;
		}
		...
		if ( ! is_dir( $parent_dir ) ) {
			if ( ! $create_directories ) {
				throw new \RuntimeException(
					...
					sprintf( __( 'Parent directory does not exist: %s', 'worker-ai' ), esc_html( $parent_dir ) )
				);
			}
		...
		if ( ! wp_is_writable( $parent_dir ) ) {
			throw new \RuntimeException(
				/* translators: %s: directory path */
				sprintf( __( 'Directory not writable: %s', 'worker-ai' ), esc_html( $parent_dir ) )
			);
		}
		...
		if ( 'append' === $mode ) {
			...
			if ( false === $result ) {
				throw new \RuntimeException(
					/* translators: %s: file path */
					sprintf( __( 'Failed to append to: %s', 'worker-ai' ), esc_html( $path ) )
				);
			}
		} else {
			if ( ! $created ) {
	...
	public function edit_file( string $path, string $old_string, string $new_string, bool $replace_all = false ): array {
		...
		if ( ! file_exists( $path ) ) {
			throw new \RuntimeException(
				/* translators: %s: file path */
				sprintf( __( 'File not found: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
		...
		if ( 0 === $count ) {
			throw new \RuntimeException( __( 'old_string not found in file.', 'worker-ai' ) );
		}
		...
		if ( ! $replace_all && $count > 1 ) {
			throw new \RuntimeException(
				sprintf(
					...
					__( 'old_string appears %d times — must appear exactly once for a safe edit, or set replace_all to true.', 'worker-ai' ),
					$count
				)
			);
		}
	...
	public function list_directory(
		...
		if ( ! is_dir( $path ) ) {
			throw new \RuntimeException(
				/* translators: %s: directory path */
				sprintf( __( 'Not a directory: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
	...
	public function delete_path( string $path, bool $recursive = false ): array {
		...
		if ( $is_dir ) {
			if ( ! $recursive ) {
				throw new \RuntimeException(
					sprintf(
						/* translators: %s: directory path */
						__( 'Path is a directory — set recursive to true to delete it: %s', 'worker-ai' ),
						esc_html( $path )
					)
				);
			}
		...
		if ( ! unlink( $path ) ) {
			throw new \RuntimeException(
				/* translators: %s: file path */
				sprintf( __( 'Failed to delete: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
	...
	/**
	 * Assert that a PHP / execution-control file lives inside WPWORKER_SANDBOX_DIR.
	 *
	...
	private function assert_sandbox_if_required( string $path ): void {
		...
		if ( ! defined( 'WPWORKER_SANDBOX_DIR' ) ) {
			throw new \InvalidArgumentException(
				__( 'PHP and execution-control files require the sandbox but WPWORKER_SANDBOX_DIR is not defined.', 'worker-ai' )
			);
		}
		...
		if ( $normalized_dir !== $normalized_box
			...
			throw new \InvalidArgumentException(
				sprintf(
					...
					__( 'PHP files can only be written to the sandbox directory: %s', 'worker-ai' ),
					esc_html( $sandbox )
				)
			);
		}
		...
		foreach ( $protected as $prot ) {
			if ( $normalized === $prot || str_starts_with( $normalized . '/', $prot . '/' ) ) {
				throw new \InvalidArgumentException(
					sprintf(
						/* translators: %s: directory path */
						__( 'Deleting this path is not allowed — it is a protected WordPress directory: %s', 'worker-ai' ),
						esc_html( $path )
					)
				);
			}
	...
	private function atomic_write( string $path, string $content ): void {
		...
		if ( false === file_put_contents( $tmp, $content ) ) {
			throw new \RuntimeException(
				/* translators: %s: file path */
				sprintf( __( 'Failed to write temp file for: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
		...
		if ( ! rename( $tmp, $path ) ) {
			...
			throw new \RuntimeException(
				/* translators: %s: file path */
				sprintf( __( 'Failed to rename temp file to: %s', 'worker-ai' ), esc_html( $path ) )
			);
		}
	}
}
