<?php
/**
 * Ability: wpworker/create-upload-link
 *
 * @package WPWorker
 * @since   1.0.0
 */
...
class CreateUploadLink extends AbstractAbility {
	...
	public function get_name(): string {
		return 'wpworker/create-upload-link';
	}

	/** {@inheritDoc} */
	public function get_label(): string {
		return __( 'Create Upload Link', 'worker-ai' );
	}

	/** {@inheritDoc} */
	public function get_description(): string {
		return __( 'Creates a temporary upload endpoint and header-only bearer token that external tools (e.g. curl) can use to upload one file into the WordPress filesystem. Accepts raw PUT/POST bodies and multipart/form-data with a field named "file".', 'worker-ai' );
	}
	...
	public function get_instructions(): string {
		return implode( "\n", [
			...
			'PHP files (*.php) and PHP execution control files can ONLY be uploaded to wp-content/wpworker-sandbox/.',
	...
	public function execute( array $input ): array|\WP_Error {
			...
			if ( ! str_starts_with( $norm_path, $norm_sand ) ) {
				return new \WP_Error(
					...
					sprintf(
						...
						__( 'PHP files and PHP execution control files can only be uploaded to the sandbox directory: %s.', 'worker-ai' ),
						$sandbox
					)
				);
			}
		...
		$upload_url   = rest_url( 'wpworker/v1/upload' );
		$token_header = 'X-WPWorker-Upload-Token';
