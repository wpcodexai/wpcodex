 ...
 *
 * Matches WPWorker's Skills page: card layout, enable/disable toggle,
...
final class SkillsPage {
	...
	public static function render(): void {
		...
		$redirect_notice = isset( $_GET['wpworker_notice'] ) ? sanitize_key( $_GET['wpworker_notice'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( 'created' === $redirect_notice ) {
			$notices[] = [ 'type' => 'success', 'message' => __( 'Skill created.', 'worker-ai' ) ];
		}
	...
	private static function handle_actions(): array {
		...
		if ( ! isset( $_POST['wpworker_skills_nonce'] ) ) {
			return $notices;
		}

		check_admin_referer( 'wpworker_skills_action', 'wpworker_skills_nonce' );
	...
	private static function handle_save(): array {
		...
		if ( ! $name || ! $description || ! $body ) {
			return [ [ 'type' => 'error', 'message' => __( 'Name, description, and body are all required.', 'worker-ai' ) ] ];
		}
		...
		if ( $is_edit ) {
			...
			return [ [ 'type' => 'success', 'message' => __( 'Skill updated.', 'worker-ai' ) ] ];
		}
		...
		wp_safe_redirect(
			add_query_arg(
				[
					'page'            => 'wpworker-skills',
					'action'          => 'edit',
					'name'            => $name,
					'wpworker_notice'  => 'created',
				],
				admin_url( 'admin.php' )
			)
		);
	...
	private static function handle_delete(): array {
		...
		return [ [ 'type' => 'success', 'message' => __( 'Skill deleted.', 'worker-ai' ) ] ];
	}
	...
	private static function handle_upload(): array {
		...
		if ( ! isset( $_FILES['skill_file'] ) || UPLOAD_ERR_OK !== $_FILES['skill_file']['error'] ) {
			return [ [ 'type' => 'error', 'message' => __( 'File upload failed.', 'worker-ai' ) ] ];
		}
		...
		if ( ! str_ends_with( $name, '.md' ) ) {
			return [ [ 'type' => 'error', 'message' => __( 'Only .md files can be uploaded as skills.', 'worker-ai' ) ] ];
		}
		...
		if ( false === $content ) {
			return [ [ 'type' => 'error', 'message' => __( 'Could not read the uploaded file.', 'worker-ai' ) ] ];
		}
		...
		if ( empty( $parsed['name'] ) || empty( $parsed['description'] ) ) {
			return [ [ 'type' => 'error', 'message' => __( 'The uploaded skill file must contain a YAML frontmatter block with at least "name" and "description" fields.', 'worker-ai' ) ] ];
		}
		...
		return [ [ 'type' => 'success', 'message' => __( 'Skill imported from file.', 'worker-ai' ) ] ];
	}
	...
	private static function handle_restore(): array {
		...
		if ( $revision_id <= 0 ) {
			return [ [ 'type' => 'error', 'message' => __( 'Invalid revision ID.', 'worker-ai' ) ] ];
		}
		...
		return [ [ 'type' => 'success', 'message' => __( 'Revision restored successfully.', 'worker-ai' ) ] ];
	}
	...
	private static function render_list( array $notices ): void {
		...
		?>
		<div class="wrap wpworker-wrap" id="wpworker-skills">
			<div class="wpworker-page-header">
				<h1 class="wpworker-page-title"><?php esc_html_e( 'Skills', 'worker-ai' ); ?></h1>
				<div class="wpworker-page-actions">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpworker-skills&action=new' ) ); ?>"
					   class="button button-primary">
						<?php esc_html_e( '+ New Skill', 'worker-ai' ); ?>
					</a>
				</div>
			</div>
			...
			<p class="wpworker-page-description">
				<?php esc_html_e( 'Skills are Markdown playbooks stored in the WordPress database. The agent reads their descriptions at session start and loads the body when the description matches the task.', 'worker-ai' ); ?>
			</p>

			<!-- Upload form -->
			<div class="wpworker-upload-area">
				<form method="post" enctype="multipart/form-data" action="">
					<?php wp_nonce_field( 'wpworker_skills_action', 'wpworker_skills_nonce' ); ?>
					<input type="hidden" name="form_action" value="upload">
					<label for="wpworker-skill-upload" class="wpworker-upload-label">
						<?php esc_html_e( 'Import skill from .md file:', 'worker-ai' ); ?>
					</label>
					<input type="file" id="wpworker-skill-upload" name="skill_file" accept=".md">
					<button type="submit" class="button"><?php esc_html_e( 'Upload', 'worker-ai' ); ?></button>
				</form>
			</div>
			...
			<?php if ( empty( $user_skills ) && empty( $external_groups ) ) : ?>
				<div class="wpworker-empty-state">
					<p><?php esc_html_e( 'No skills yet. Create your first skill or upload a .md file.', 'worker-ai' ); ?></p>
				</div>
			<?php else : ?>
				<?php if ( ! empty( $user_skills ) ) : ?>
					<div class="wpworker-cards">
	...
	private static function render_edit( string $action, array $notices ): void {
		...
		$title = 'edit' === $action ? __( 'Edit Skill', 'worker-ai' ) : __( 'New Skill', 'worker-ai' );
		?>
		<div class="wrap wpworker-wrap" id="wpworker-skill-editor">
			<div class="wpworker-page-header">
				<h1 class="wpworker-page-title"><?php echo esc_html( $title ); ?></h1>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpworker-skills' ) ); ?>"
				   class="button">&larr; <?php esc_html_e( 'Back to Skills', 'worker-ai' ); ?></a>
			</div>
			<div class="wp-header-end"></div>
			<?php self::render_notices( $notices ); ?>
			...
			<form method="post" action="<?php echo esc_url( $self_url ); ?>">
				...
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="skill_name"><?php esc_html_e( 'Name (slug)', 'worker-ai' ); ?></label></th>
						<td>
							<input
								type="text"
								id="skill_name"
								...
								required
							>
							<p class="description"><?php esc_html_e( 'Lowercase, hyphen-separated. Cannot be changed after creation.', 'worker-ai' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="skill_description"><?php esc_html_e( 'Description (trigger)', 'worker-ai' ); ?></label></th>
						<td>
							<input
								type="text"
								...
								required
							>
							<p class="description"><?php esc_html_e( 'The trigger sentence the agent reads at session start. Write it so the agent knows when to use this skill.', 'worker-ai' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="skill_body"><?php esc_html_e( 'Body (Markdown)', 'worker-ai' ); ?></label></th>
						<td>
							<textarea
								id="skill_body"
								name="skill_body"
								rows="20"
								class="large-text code wpworker-skill-editor"
								required
							><?php echo esc_textarea( $skill['body'] ?? '' ); ?></textarea>
							<p class="description"><?php esc_html_e( 'Plain Markdown. This is loaded into the agent\'s context when the description matches.', 'worker-ai' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Agentic mode', 'worker-ai' ); ?></th>
						<td>
							<label>
								...
								<?php esc_html_e( 'Fire automatically when description matches (enable_agentic)', 'worker-ai' ); ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Prompt menu', 'worker-ai' ); ?></th>
						<td>
							<label>
								...
								<?php esc_html_e( 'Show in AI client prompt menu (enable_prompt)', 'worker-ai' ); ?>
							</label>
						</td>
					</tr>
				</table>

				<?php submit_button( $skill ? __( 'Update Skill', 'worker-ai' ) : __( 'Create Skill', 'worker-ai' ) ); ?>
			</form>
			...
			<?php if ( 'edit' === $action && null !== $skill ) : ?>
				...
				if ( ! empty( $revisions ) ) :
				?>
				<div class="wpworker-revisions" style="margin-top: 2em;">
					<h2><?php esc_html_e( 'Revision History', 'worker-ai' ); ?></h2>
					<p class="description"><?php esc_html_e( 'Up to 10 previous versions. Restoring a revision snapshots the current state first, so every restore is reversible.', 'worker-ai' ); ?></p>
					<table class="widefat striped" style="margin-top: 0.75em;">
						<thead>
							<tr>
								<th><?php esc_html_e( '#', 'worker-ai' ); ?></th>
								<th><?php esc_html_e( 'Saved', 'worker-ai' ); ?></th>
								<th><?php esc_html_e( 'Description', 'worker-ai' ); ?></th>
								<th><?php esc_html_e( 'Body preview', 'worker-ai' ); ?></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<?php foreach ( $revisions as $index => $rev ) : ?>
								<tr>
									...
									<td>
										<form method="post"
										      action="<?php echo esc_url( admin_url( 'admin.php?page=wpworker-skills&action=edit&name=' . urlencode( (string) $skill['name'] ) ) ); ?>"
											...
											<button type="submit"
											        ...
											        onclick="return confirm('<?php echo esc_js( __( 'Restore this revision? The current state will be saved first.', 'worker-ai' ) ); ?>')">
												<?php esc_html_e( 'Restore', 'worker-ai' ); ?>
											</button>
										</form>
									</td>
								</tr>
	...
	private static function render_skill_card( array $skill, bool $readonly = false ): void {
		...
		$edit_url       = admin_url( 'admin.php?page=wpworker-skills&action=edit&name=' . urlencode( $name ) );
		?>
		<div class="wpworker-card<?php echo $readonly ? ' wpworker-card--readonly' : ''; ?>">
			<div class="wpworker-card__header">
				<span class="wpworker-card__name"><?php echo esc_html( $name ); ?></span>
				<div class="wpworker-card__badges">
					<?php if ( $enable_agentic ) : ?>
						<span class="wpworker-badge wpworker-badge--agentic"><?php esc_html_e( 'Agentic', 'worker-ai' ); ?></span>
					<?php endif; ?>
					<?php if ( $enable_prompt ) : ?>
						<span class="wpworker-badge wpworker-badge--prompt"><?php esc_html_e( 'Prompt', 'worker-ai' ); ?></span>
					<?php endif; ?>
				</div>
			</div>
			<p class="wpworker-card__description"><?php echo esc_html( $description ); ?></p>
			<?php if ( ! $readonly ) : ?>
			<div class="wpworker-card__actions">
				<a href="<?php echo esc_url( $edit_url ); ?>" class="button button-small">
					<?php esc_html_e( 'Edit', 'worker-ai' ); ?>
				</a>
				<form method="post" action="" style="display:inline;">
					...
					<button type="submit" class="button button-small button-link-delete"
					        onclick="return confirm('<?php echo esc_js( __( 'Delete this skill? This cannot be undone.', 'worker-ai' ) ); ?>')">
						<?php esc_html_e( 'Delete', 'worker-ai' ); ?>
					</button>
				</form>
			</div>
			<?php endif; ?>
